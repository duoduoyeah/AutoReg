...
class RegressionModel(BaseModel):
    type: str = Field(
        default="basic_regression", description="Type of regression model"
    )
    dependent_vars: list[str] = Field(
        description="List of dependent variables", default_factory=list
    )
    independent_vars: list[str] = Field(
        description="List of independent variables", default_factory=list
    )
    effects: list[str] = Field(
        description="Fixed effects to include in regression, at most two effects",
        default_factory=list,
    )
    control_vars: list[str] = Field(
        description="List of control variables", default_factory=list
    )
...
class ResearchTopic(BaseModel):
    ...
    dependent_var_name: str = Field(
        description="Name of dependent variable", default=""
    )
    dependent_var_description: str = Field(
        description="Description of dependent variable", default=""
    )
    independent_var_name: str = Field(
        description="The only one name of independent variable", default=""
    )
    independent_var_description: str = Field(
        description="The only one description of independent variable", default=""
    )


class ControlVariables(BaseModel):
    control_vars_name: list[str] = Field(
        description="List of control variables", default_factory=list
    )
    control_vars_description: list[str] = Field(
        description="List of control variables", default_factory=list
    )
...
class NewVariable(BaseModel):
    ...
    is_dummy: bool = Field(
        description="Whether the new variable is a dummy variable", default=False
    )
    new_variable_description: str = Field(
        description="Description of new variable", default=""
    )
    most_relevant_variable: str = Field(
        description="Most relevant variable - either dependent var, independent var, or None",
        default="None",
    )
...
def generate_research_topic(
    ...
    query = LangchainQueries.format_query(
        LangchainQueries.RESEARCH_TOPIC_PROMPT, research_topic=description
    )
...
def generate_control_variables(
    ...
    independent_variable = (
        research_topic["independent_var_name"]
        + " "
        + research_topic["independent_var_description"]
    )
    dependent_variable = (
        research_topic["dependent_var_name"]
        + " "
        + research_topic["dependent_var_description"]
    )
