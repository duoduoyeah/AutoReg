...
def generate_variables(
    ...
    if related_var is not None and correlation is not None:
        ...
        entity_effect = np.random.normal(
            0, 0.4, len(df.index.get_level_values("entity").unique())
        )
        time_effect = np.random.normal(
            0, 0.4, len(df.index.get_level_values("time").unique())
        )

        # Map effects to each observation
        entity_component = pd.Series(
            entity_effect[df.index.get_level_values("entity")], index=df.index
        )
        time_component = pd.Series(
            time_effect[df.index.get_level_values("time")], index=df.index
        )

        # Combine random effect with entity and time components (0.8 weight on random component)
        df[var_name] = (
            np.random.normal(0, 0.2, n_samples) + entity_component + time_component
        )
...
def generate_new_csv(n_entities: int = 100, n_periods: int = 10):
    ...
    df = generate_variables(
        var_name="stock_revenue", related_var="extreme_temperature", df=df
    )
    # control variables
    control_vars = [
        "company_size",
    ...
    for control_var in control_vars:
        df = generate_variables(
            var_name=control_var, related_var="stock_revenue", correlation=0.1, df=df
        )

    # tool variable
    df = generate_variables(
        var_name="latitude", related_var="extreme_temperature", correlation=0.6, df=df
    )

    # heterogenous variable
    df = generate_variables(
        var_name="is_high_tech",
        related_var="stock_revenue",
        correlation=0.4,
        df=df,
        dummy_var=True,
    )
    df = generate_variables(
        var_name="is_near_sea",
        related_var="stock_revenue",
        correlation=0.1,
        df=df,
        dummy_var=True,
    ...
    # stability test
    df = generate_variables(
        var_name="extreme_temperature_another_measure_method",
    ...
    df = generate_variables(
        var_name="stock_revenue_another_measure_method",
        related_var="stock_revenue",
        correlation=0.9,
        df=df,
    )
