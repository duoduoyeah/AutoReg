...
class ResultTables(BaseModel):
    tables: list[RegressionResultTable|None] = Field(default_factory=list) # the regression result tables
    description: list[str] = Field(default=[])  # the description of the regression results that are used to create the tables
    analysis: list[RegressionAnalysis|None] = Field(description="the analysis of the regression results that are used to create the tables", default=[])
